...
class LpAVFSortCompat(object):
    ...
    def add_variables_cplex(self, aids):
        ...
        self.lp.variables.add(names = ['y_' + aid for aid in aids],
                              lb = [0 for aid in aids],
                              ub = [1 for aid in aids])
        self.lp.variables.add(names = ['alpha'], lb = [0], ub = [1])
    ...
    def encode_constraint_cplex(self, aa, ap):
        ...
        if cat_nr < len(self.cat):
            ...
            constraints.add(names = ["alpha_x_%s" % aa.id],
                            lin_expr =
                                [[
                                  ['x_' + aa.id, "alpha"],
                                  [1, -1],
        ...
        if cat_nr > 1:
            ...
            constraints.add(names = ["alpha_y_%s" % aa.id],
                            lin_expr =
                                [[
                                  ['y_' + aa.id, "alpha"],
                                  [1, -1],
    ...
    def add_variables_glpk(self, aids):
        ...
        self.alpha = self.lp.var(name = 'alpha', bounds=(0, 1))
    ...
    def encode_constraints_glpk(self, aas, pt):
        ...
        # sum ((sum w_it) + k * w_it+1) - u_k + x_j <= 0
        # sum ((sum w_it) + k * w_it+1) - u_k - y_j >= 0
        for i, aa in enumerate(aas):
            ...
            if cat_nr < len(self.cat):
                ...
                self.lp.st(self.x[i] - self.alpha <= 0)
            ...
            if cat_nr > 1:
                ...
                self.lp.st(self.y[i] - self.alpha <= 0)
