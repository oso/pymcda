...
class electre_tri_tests(unittest.TestCase):
    ...
    def test001_test_pessimist_loulouka(self):
        """ Loulouka - Pessimist """
        from data_loulouka import c, cv, ptb, lbda, pt, aap, cats
        etri = electre_tri(c, cv, ptb, lbda, cats).pessimist(pt)
    ...
    def test002_test_pessimist_ticino(self):
        """ Ticino - Pessimist """
        from data_ticino import c, cv, ptb, lbda, pt, aap, cats
        etri = electre_tri(c, cv, ptb, lbda, cats).pessimist(pt)
        ok = compare_affectations(etri, aap)
        self.assertEqual(ok, 1, "One or more affectations were wrongly \
                         assigned")
    ...
    def test003_test_optimist_loulouka(self):
        """ Loulouka - Optimist """
        from data_loulouka import c, cv, ptb, lbda, pt, aao, cats
        etri = electre_tri(c, cv, ptb, lbda, cats).optimist(pt)
    ...
    def test004_test_optimist_ticino(self):
        """ Ticino - Optimist """
        from data_ticino import c, cv, ptb, lbda, pt, aao, cats
        etri = electre_tri(c, cv, ptb, lbda, cats).optimist(pt)
        ok = compare_affectations(etri, aao)
        self.assertEqual(ok, 1, "One or more affectations were wrongly \
                         assigned")
